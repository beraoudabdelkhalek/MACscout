...
def is_valid_mac_address(mac_address):
    regex = re.compile(r'^([0-9A-F]{2}[:-]){5}([0-9A-F]{2})$')
    return regex.match(mac_address) is not None

def is_valid_oui(oui):
    regex = re.compile(r'^([0-9A-F]{2}[:-]){2}([0-9A-F]{2})$')
...
def getVendorStrict(mac):
    ...
    for element in data:
        if element["macPrefix"]==macadr:
            return element["vendorName"]
    return "not found"
#get the MAC prefix of the vendor using the vendor's name
